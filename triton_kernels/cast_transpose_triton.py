        ...
        for output_type in output_types:
            ...
            for M, N in shapes:
                ...
                if not torch.allclose(torch_c.to(torch.float32), triton_c.to(torch.float32), atol=1e-2):
                    ...
                    print(f"torch_c= {torch_c.to(torch.float32)}\n, triton_c={triton_c.to(torch.float32)}")
                    ref_outc = torch_c.to(torch.float32).view(-1)
                    outc = triton_c.to(torch.float32).view(-1)
                    import pdb
                    pdb.set_trace()
