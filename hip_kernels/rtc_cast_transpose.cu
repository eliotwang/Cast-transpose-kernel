...
__global__ void __launch_bounds__(block_size) cast_transpose_optimized_kernel(
  ...
  //constexpr size_t tile_dim_n = warps_per_tile * iter_size * nvec_in;
  ...
  // load/store
  constexpr size_t num_iterations = iter_size; //HIP_THREADS_PER_WARP / warps_per_tile;
...
#pragma unroll
  ...
  __shared__ OVecT shared_output_t[HIP_THREADS_PER_WARP][warps_per_tile*iter_size + 1];
#pragma unroll
  for (size_t j2 = 0; j2 < nvec_in; ++j2) {
#pragma unroll
    for (size_t iter = 0; iter < num_iterations; ++iter) {
    ...
    __syncthreads();
#pragma unroll
    for (size_t iter = 0; iter < (HIP_THREADS_PER_WARP/warps_per_tile); ++iter) {
