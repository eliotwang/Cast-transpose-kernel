...
using CType = float;
constexpr size_t HIP_THREADS_PER_WARP = THREADS_PER_WARP;
constexpr size_t warps_per_tile = 4;
constexpr size_t block_size = HIP_THREADS_PER_WARP * warps_per_tile;
...
struct KernelConfig {
  ...
  KernelConfig(size_t row_length, size_t num_rows, size_t itype_size, size_t otype_size,
               size_t load_size_, size_t store_size_, size_t wpt_size_, size_t iter_size_)
      : load_size{load_size_}, store_size{store_size_}, wpt_size{wpt_size_}, iter_size{iter_size_} {
    ...
    const size_t row_tile_elements = load_size * HIP_THREADS_PER_WARP / itype_size;
    ...
    active_sm_count = std::min(DIVUP(num_blocks * wpt_size, warps_per_sm),
...
__global__ void __launch_bounds__(block_size)
  ...
  constexpr size_t bdimx = HIP_THREADS_PER_WARP;
  ...
  constexpr size_t tile_dim_m = HIP_THREADS_PER_WARP * nvec_out;
  constexpr size_t tile_dim_n = HIP_THREADS_PER_WARP * nvec_in;
  ...
  // load/store
  constexpr size_t num_iterations = HIP_THREADS_PER_WARP / warps_per_tile;
...
#pragma unroll
  ...
  __shared__ OVecT shared_output_t[HIP_THREADS_PER_WARP][HIP_THREADS_PER_WARP + 1];
#pragma unroll
  for (size_t j2 = 0; j2 < nvec_in; ++j2) {
#pragma unroll
    for (size_t iter = 0; iter < num_iterations; ++iter) {
...
void cast_transpose(const Tensor &input, const Tensor &noop, Tensor *cast_output_,
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          ...
          const bool aligned =
              (row_length % HIP_THREADS_PER_WARP == 0 && num_rows % HIP_THREADS_PER_WARP == 0);
          if (aligned && rtc::is_enabled()) {  // Runtime-compiled tuned kernel
            // Pick kernel config
            std::vector<KernelConfig> kernel_configs;
            kernel_configs.reserve(576);
            auto add_config = [&](size_t load_size, size_t store_size, size_t wpt_size, size_t iter_size) {
              kernel_configs.emplace_back(row_length, num_rows, itype_size, otype_size, load_size, store_size, wpt_size, iter_size);
            };
            
            
            for(int ld_size = itype_size; ld_size <= 8; ld_size*=2)
              for(int st_size = otype_size; st_size <=8; st_size*=2)
                for(int wpt_size = 4; wpt_size <=16; wpt_size *=2)
                {
                  int iter_end = HIP_THREADS_PER_WARP / wpt_size;
                    for(int iter_size = 1; iter_size <= iter_end; iter_size*=2)
                    {
                      add_config(ld_size, st_size, wpt_size, iter_size);
                    }
                }
           ...
           // const auto &kernel_config =
            ...
            {
              printf("kernel_config: load_size = %d, store_size =%d, wpt_size = %d, iter_size = %d\n", kernel_config.load_size, kernel_config.store_size, kernel_config.wpt_size, kernel_config.iter_size);
              //NVTE_CHECK(kernel_config.valid, "invalid kernel config");
              if(kernel_config.valid == false){
              ...
              const size_t wpt_size = kernel_config.wpt_size;
              const size_t iter_size = kernel_config.iter_size;
              const size_t num_blocks = kernel_config.num_blocks;
              const size_t rtc_block_size = HIP_THREADS_PER_WARP * wpt_size;
              ...
              printf("current kernel config execution time: %f\n", (milliseconds / iters) * 1000);
            }

          } else {  // Statically-compiled general kernel
            constexpr size_t load_size = 4;
            constexpr size_t store_size = 4;
            constexpr size_t row_tile_size = load_size / itype_size * HIP_THREADS_PER_WARP;
            constexpr size_t col_tile_size = store_size / otype_size * HIP_THREADS_PER_WARP;
